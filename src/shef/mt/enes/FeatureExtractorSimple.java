...
import shef.mt.tools.CCGprocessor;
...
public class FeatureExtractorSimple{
 ...
 private static PropertiesManager resourceManager;
    ...
    public static void main(String[] args) {
        ...
        if (resourceManager.getString("mode").equals("ccg"))
                fe.runCCG();
        else
                fe.run();
...
*/
     ...
     */
    public void runCCG(){
        String CCGworkdir=resourceManager.getString("CCGworkdir");
         File f = new File(sourceFile);
        String sourceFileName = f.getName();
        f = new File(targetFile);
        String targetFileName = f.getName();
        ...
        String lineSource = brSource.readLine();
        String lineTarget = brTarget.readLine();
       // System.out.println(" Source + target"+lineSource+"\n"+lineTarget);
        Sentence sourceSent;
        Sentence targetSent;
        int sentCount = 0;    
            ...
            while ((lineSource != null) && (lineTarget != null)) {
                ...
                CCGprocessor ccgproc=new CCGprocessor(CCGworkdir,resourceManager.getString("CCGresourcedir"),sourceSent,targetSent,resourceManager.getString("tools.ngram.path"),resourceManager.getString(targetLang + ".superlm"));

                ccgproc.processNextSentence();
